  ...
  setTimeout(function() {
    $.get('./control/get.php', { id: id },
      function ( response ) {
        ...
        $("input[name='nombre']").val( fromHTML(r.prod_nombre ) );
        $("input[name='resumen']").val( fromHTML( r.prod_resumen ) );
        $("input[name='video']").val( r.prod_video );
        $("select[name='marca']").val( r.prod_marca );
        ...
        $("input[name='caracteristicas']").fileinput({
          ...
          initialPreviewAsData: true,
          initialPreviewFileType: 'image',
          initialPreview: '../../img/productos/'+r.prod_caracteristicas
        });
        ...
        CKEDITOR.instances["contenido"].setData(r.prod_descripcion);
...
function llenarTabla() {
  $.get('./control/lista.documentos.php',
    { id: IDPROD },
    function (response) {
      $("tbody").html(response);
    }
